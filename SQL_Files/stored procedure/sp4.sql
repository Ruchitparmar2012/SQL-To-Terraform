CREATE OR REPLACE PROCEDURE DW_DEV.INTEGRATION.GET_KEYRING_DATA_BY_JAVA_TEST("INPUT_DATA" VARIANT)
RETURNS VARIANT
LANGUAGE JAVASCRIPT
EXECUTE AS OWNER
AS '
async function check_values(input_data) {
  const snowflakeSession = snowflake.createStatement({ sqlText: "SELECT COLUMN_NAME FROM DW_DEV.INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = ''EMPLOYEE_CROSS_WALK'' AND TABLE_SCHEMA = ''INTEGRATION''" });
...
async function get_ams_employee_id(condition_statement) {
  ...
  FROM DW_DEV.INTEGRATION.EMPLOYEE_CROSS_WALK_E2E_TENANT CW
