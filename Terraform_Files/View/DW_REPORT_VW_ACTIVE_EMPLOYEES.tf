resource "snowflake_secure_view" "DW_REPORT_VW_ACTIVE_EMPLOYEES" {
...
(
	SELECT DISTINCT NVL( e1.EMPLOYEE_KEY, e2.EMPLOYEE_KEY) AS EMPLOYEE_KEY,
		...
		e1.SOURCE_SYSTEM_ID
	FROM (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE WHERE SOURCE_SYSTEM_ID IN ('5')) e1
	FULL OUTER JOIN (SELECT * FROM DW_${var.SF_ENVIRONMENT}.HAH.DIM_EMPLOYEE WHERE SOURCE_SYSTEM_ID IN ('1','2','3','4')) e2
...
(
	...
	FROM DISC_${var.SF_ENVIRONMENT}.GPSYNCDATA.PAYSTUB_MASTER c
...
(
	...
	FROM DW_${var.SF_ENVIRONMENT}.HAH.FACT_PAYROLL p
	INNER JOIN DIMDATE dd
...
(
	...
	LEFT OUTER JOIN DW_${var.SF_ENVIRONMENT}.HAH.DIM_BRANCH b
