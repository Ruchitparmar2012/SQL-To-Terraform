resource "snowflake_secure_view" "DW_INTEGRATION_VW_EMPLOYEE_CROSS_WALK" {
	database = "DW_${var.SF_ENVIRONMENT_dev}"
	schema = "INTEGRATION"
...
WITH MAPPED_EMPLOYEES AS 
	(
	    ...
	    FULL OUTER JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_APPLICANT_EMPLOYEE_MAPPING APP 
...
, FINAL AS (
        ...
        FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.DIM_EMPLOYEE_MERGED EM
...
--            AND EM.EMPLOYEE_KEY NOT IN (SELECT DISTINCT PAYROLL_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_AMS_PAYROLL_MAPPING )
...
--		'e725e24564921231af03299952620ab8') 
    ...
    -------------------
        ...
        LEFT JOIN DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_APPLICANT_EMPLOYEE_MAPPING APP 
        ...
        AND EM.EMPLOYEE_KEY NOT IN (SELECT DISTINCT AMS_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_AMS_PAYROLL_MAPPING)
--        AND EM.EMPLOYEE_KEY NOT IN (SELECT DISTINCT AMS_EMPLOYEE_KEY FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_AMS_PAYROLL_MAPPING)
    ...
    UNION
        ...
        WHERE
            ...
            AND STATUS.APPLICANT_KEY NOT IN ( SELECT DISTINCT APPLICANT_KEY FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.FACT_APPLICANT_EMPLOYEE_MAPPING APP ) -- 3223
)
