resource "snowflake_procedure" "DW_INTEGRATION_GET_EMPLOYEE_KEYRING_DATA_FOR_CONFIG_TENANT" {
	name ="GET_EMPLOYEE_KEYRING_DATA_FOR_CONFIG_TENANT"
	...
	execute_as = "OWNER"
	runtime_version = "3.8"
	packges = "["SNOWFLAKE-SNOWPARK-PYTHON","PANDAS","SIMPLEJSON"]"
...
def get_ams_employee_id(snowpark_session,condition_statement : str ):
  ...
  try:
      results_df = snowpark_session.sql(f''''''SELECT  object_agg(EMPLOYEE_ENTERPRISE_ID,  object_construct_keep_null(
                    ...
                    FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.EMPLOYEE_CROSS_WALK_14APR23_CONFIG_TENANT CW
