resource "snowflake_procedure" "DW_INTEGRATION_GET_AMS_EMPLOYEE_ID" {
	name ="GET_AMS_EMPLOYEE_ID"
...
LET OP_RESULT RESULTSET := 
			...
			WHEN :EMPLOYEE_ENTERPRISE_ID NOT IN 
                (SELECT DISTINCT EMPLOYEE_ENTERPRISE_ID FROM DW_${var.SF_ENVIRONMENT}.INTEGRATION.EMPLOYEE_CROSS_WALK)
